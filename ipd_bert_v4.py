...
# For v4: we use regression features directly in the BERT model to predict the last choice, incorporating engineered features into the transformer architecture.
input_length = 8
prediction_length = 1
...
## IPD Task v4 - Predict last move using sequence + regression features
...
def ipd_regression_data_v4(regressiondata):
    """Create regression data for v4: use 8 moves to predict the 9th move with enhanced features"""
    x, y = [], []
    for e in regressiondata:
        # Enhanced features: previous 8 moves from both players + additional features
        features = []
...
def valid_ipd_v4(n):
    """Load and prepare data for v4 prediction task"""
    if n < 1:
        n = int(8258 * n)
    ...
    # Create regression data for v4
    train_set_rgx, train_set_rgy = ipd_regression_data_v4(regressiondata[n:])
    test_set_rgx, test_set_rgy = ipd_regression_data_v4(regressiondata[:n])
...
def ipd_set2arset_v4():
    """Prepare AR data for v4: use 8 moves to predict the 9th move"""
...
class bertModel_v4(nn.Module):
    def __init__(self, seq_dim, feature_dim, hidden_dim, out_dim, num_layers, num_heads=2, dropout=0.1):
        super().__init__()
        ...
        # Sequence embedding (for the 8 moves)
        self.seq_embedding = nn.Linear(seq_dim, hidden_dim)
        
        # Feature embedding (for regression features)
    ...
    def forward(self, seq_data, feature_data):
        # seq_data shape: (batch_size, seq_len, seq_dim) - the 8 moves
        # feature_data shape: (batch_size, feature_dim) - regression features
        
        batch_size, seq_len, _ = seq_data.shape
        
        # Embed sequence data
        seq_embedded = self.seq_embedding(seq_data)  # (batch_size, seq_len, hidden_dim)
        
        # Embed feature data and expand to match sequence length
        feature_embedded = self.feature_embedding(feature_data)  # (batch_size, hidden_dim)
        feature_embedded = feature_embedded.unsqueeze(1).expand(-1, seq_len, -1)  # (batch_size, seq_len, hidden_dim)
        
        # Combine sequence and feature embeddings
        combined = seq_embedded + feature_embedded  # (batch_size, seq_len, hidden_dim)
        
        # Add positional encoding
        combined = combined.transpose(0, 1)  # (seq_len, batch_size, hidden_dim)
        combined = self.positional_encoding(combined)
        combined = combined.transpose(0, 1)  # (batch_size, seq_len, hidden_dim)
        ...
        pooled = torch.mean(combined, dim=1)  # (batch_size, hidden_dim)
        
        # Output layer
        output = self.output_layer(pooled)  # (batch_size, out_dim)
...
for fold in np.arange(n_fold):
    ...
    with open("data/IPD/processed_train_test_v4.pkl", "wb") as handle:
        pickle.dump(full_data, handle, protocol=pickle.HIGHEST_PROTOCOL)
    n_nodes, n_layers = 10, 2
    
    # Train LSTM model (sequence only)
    ...
    for ep in np.arange(n_epochs):
        for bc in np.arange(train_set.shape[0] / batch_size):
            ...
            pred = output[:, -1, :]  # (batch_size, 2) - probabilities for both actions
            # Convert to binary prediction (probability of cooperation)
            pred_coop = pred[:, 0]  # Probability of cooperation
            
            loss = criterion_lstm(pred_coop, target.squeeze())
    ...
    # Train BERT v4 model (sequence + features)
    bert = bertModel_v4(2, train_set_rgx.shape[1], n_nodes, 2, n_layers).to(device)
    criterion_bert = nn.MSELoss()
    optimizer_bert = optim.Adam(bert.parameters(), lr=1e-4, weight_decay=1e-4)
    loss_set_bert = []
    val_loss_set_bert = []
    
    print("Training BERT v4 model...")
    for ep in np.arange(n_epochs):
        bert.train()
        epoch_losses = []
        
        for bc in np.arange(train_set.shape[0] / batch_size):
            batch_start = int(bc * batch_size)
            batch_end = int((bc + 1) * batch_size)
            
            # Sequence data: first 8 moves
            seq_inputs = Variable(
                torch.from_numpy(train_set[batch_start:batch_end])
                .transpose(1, 2)
                .float()
                .to(device)
            )
            seq_data = seq_inputs[:, :input_length, :]  # (batch_size, 8, 2)
            
            # Feature data: regression features
            feature_data = Variable(
                torch.from_numpy(train_set_rgx[batch_start:batch_end])
                .float()
                .to(device)
            )  # (batch_size, feature_dim)
            
            # Target: 9th move (position 8)
            target = seq_inputs[:, input_length, 0].unsqueeze(1)  # (batch_size, 1)
            
            output = bert(seq_data, feature_data)
            # Output is (batch_size, 2) - probabilities for both actions
            pred_coop = output[:, 0]  # Probability of cooperation
            
            loss = criterion_bert(pred_coop, target.squeeze())
            ...
            if bc % window == 0:
                print(fold)
                print(
                    "BERT v4 Epoch[{}/{}], Batch[{}/{}], Loss: {:.5f}".format(
                        ep + 1,
                        n_epochs,
                        bc + 1,
                        train_set.shape[0] / batch_size,
        ...
        bert.eval()
        with torch.no_grad():
            # Validation sequence data
            val_seq_inputs = Variable(torch.from_numpy(test_set).transpose(1, 2).float().to(device))
            val_seq_data = val_seq_inputs[:, :input_length, :]  # First 8 moves
            
            # Validation feature data
            val_feature_data = Variable(torch.from_numpy(test_set_rgx).float().to(device))
            
            val_output = bert(val_seq_data, val_feature_data)
            val_pred = val_output[:, 0]  # Probability of cooperation for 9th move
            val_targ = val_seq_inputs[:, input_length, 0]  # Actual 9th move
            val_loss = criterion_bert(val_pred, val_targ).item()
            val_loss_set_bert.append(val_loss)
            
            if ep % 2 == 0:  # Print every 2 epochs
                print(f"BERT v4 Epoch {ep+1}: Train Loss: {np.mean(epoch_losses):.5f}, Val Loss: {val_loss:.5f}")
    
    bert = bert.eval()
    
    # Save the trained BERT model
    torch.save(bert.state_dict(), f"bert_model_v4_fold_{fold}.pth")
    print(f"BERT v4 model saved for fold {fold}")
    
    # AR model
    train_arset = ipd_set2arset_v4()
    ...
    # For BERT v4: use first 8 moves + features to predict 9th move
    input_bert_seq = varX[:, :input_length, :]
    input_bert_features = Variable(torch.from_numpy(test_set_rgx).float().to(device))
    py_bert_raw = bert(input_bert_seq, input_bert_features).data.cpu().numpy()
    py_bert = py_bert_raw[:, 0]  # Probability of cooperation for 9th move
...
pyc_bert = py_bert  # BERT v4 prediction for 9th move
...
print(f"BERT v4 MSE: {bert_mse:.6f}")
...
import os
os.makedirs("Figures/IPD_v4", exist_ok=True)

# Plot cooperation rates comparison
plt.clf()
plt.figure(figsize=(10, 6))
models = ['LSTM', 'BERT v4', 'AR', 'LR', 'Human']
...
plt.title('IPD v4 - 9th Move Cooperation Prediction (Sequence + Features)')
plt.xticks(x_pos, models)
plt.ylim([0, 1])
plt.tight_layout()
plt.savefig(f"Figures/IPD_v4/ipd_v4_coop_comparison_nodes_{n_nodes}_layers_{n_layers}.png")

# Plot MSE comparison
plt.clf()
plt.figure(figsize=(10, 6))
mse_values = [lstm_mse, bert_mse, ar_mse, lr_mse]
mse_models = ['LSTM', 'BERT v4', 'AR', 'LR']

x_pos = np.arange(len(mse_models))
plt.bar(x_pos, mse_values, alpha=0.7)
plt.xlabel('Models')
plt.ylabel('MSE')
plt.title('IPD v4 - 9th Move Prediction MSE (Sequence + Features)')
plt.xticks(x_pos, mse_models)
plt.tight_layout()
plt.savefig(f"Figures/IPD_v4/ipd_v4_mse_comparison_nodes_{n_nodes}_layers_{n_layers}.png")

# Plot training losses
plt.clf()
plt.plot(loss_set_lstm, label='LSTM Loss')
plt.plot(loss_set_bert, label='BERT v4 Loss')
plt.xlabel('Batch')
plt.ylabel('Loss')
plt.title('IPD v4 - Training Loss (Sequence + Features)')
plt.legend()
plt.tight_layout()
plt.savefig(f"Figures/IPD_v4/ipd_v4_training_loss_nodes_{n_nodes}_layers_{n_layers}.png")

# Plot validation loss for BERT
plt.clf()
plt.plot(val_loss_set_bert, 'r-', label='BERT v4 Validation Loss')
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.title('IPD v4 - BERT v4 Validation Loss')
plt.legend()
plt.tight_layout()
plt.savefig(f"Figures/IPD_v4/ipd_v4_bert_val_loss_nodes_{n_nodes}_layers_{n_layers}.png")

print(f"BERT v4 vs LSTM improvement: {(lstm_mse - bert_mse) / lstm_mse * 100:.2f}%")
print(f"BERT v4 vs AR improvement: {(ar_mse - bert_mse) / ar_mse * 100:.2f}%")
print(f"BERT v4 vs LR improvement: {(lr_mse - bert_mse) / lr_mse * 100:.2f}%")

# Additional analysis: feature importance
print(f"\nFeature Analysis:")
print(f"Sequence dimension: {input_length * 2} (8 moves × 2 players)")
print(f"Feature dimension: {train_set_rgx.shape[1]} (regression features)")
print(f"Total input features: {input_length * 2 + train_set_rgx.shape[1]}") 